...
import VolunteerApplicationStore from 'app/stores/volunteerApplicationStore';
...
export default class VolunteerApplication extends React.Component {
    ...
    updateTable = () => {
        this.setState({
            data: VolunteerApplicationStore.getAll()
        });
    };
    ...
    componentWillMount () {
        VolunteerApplicationStore.on('update', this.updateTable);
        VolunteeringActions.fetchData();
    }

    componentWillUnmount () {
        VolunteerApplicationStore.removeListener('update', this.updateTable);
    }
